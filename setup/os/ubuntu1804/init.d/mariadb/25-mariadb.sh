...
if [[ ! -z "$MARIADB_ROOT_USERNAME" && ! -z "$MARIADB_ROOT_PASSWORD" ]]; then
  # Create root user and set the password.
  sudo mysql --protocol=socket <<EOF
  CREATE USER IF NOT EXISTS "$MARIADB_ROOT_USERNAME"@'$MARIADB_HOST';
  ALTER USER "$MARIADB_ROOT_USERNAME"@'$MARIADB_HOST' IDENTIFIED BY '$MARIADB_ROOT_PASSWORD';
  GRANT ALL PRIVILEGES ON *.* TO "$MARIADB_ROOT_USERNAME"@'$MARIADB_HOST';
EOF
fi
